...
`config/settings.yaml` 的 `review.auto_below`（預設 50）：測試跑完若**總成功率低於此 %**，會自動叫 headless Claude 看 `diagnose` + 失敗截圖、**修正腳本並推 git**（只修一次，不自動重跑）。設 `0` 關閉，或 `test` 加 `--no-review` 單次略過。需 `claude` CLI 已登入。
...
|---|---|---|
| `tap` | 點擊比例座標 | `x`, `y` |
| `tap_image` | 在當前畫面找到該元件再點它（跨解析度最準） | `template`, `timeout`, `region` |
| `tap_scene` | 比對畫面與錄影相符才點同位置，不符不點 | `reference`, `x`, `y`, `scene_threshold`(預設0.65) |
