...
6. **點擊策略（重要，避免猜座標）**：核心原則是「比對目前畫面與錄影當下相同 → 點同一位置」。
   - **優先 `tap_image`**：把要點的按鈕/圖示用 cv2 裁成模板存 `assets/`，runtime 在當前畫面找到該元件就點它的中心（跨解析度自動定位，最準）。**能裁模板的互動一律用這個。**
   - 沒有明顯按鈕、或要點空白/固定位置 → 用 **`tap_scene`**：附 `reference`（錄影當下整張畫面）+ 正規化 `x,y`；runtime 會先比對目前畫面與 reference 相似度，**相符才點該座標**，不符就報錯不亂點。
   - **不要產生沒有任何畫面驗證的純 `tap` 盲點座標**（那就是先前 [需校正] 不準的來源）。真的只能估座標時，改成 `tap_scene` 附上該步的參考影格。
